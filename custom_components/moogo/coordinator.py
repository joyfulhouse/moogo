...
class MoogoCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        self, hass: HomeAssistant, api: MoogoClient, entry: ConfigEntry
    ) -> None:
        ...
        try:
            ...
            if self.api.is_authenticated:
                ...
                if new_interval != self.update_interval.total_seconds():
                    ...
                    _LOGGER.info(
                        f"Updated polling interval to {new_interval} seconds for authenticated data"
                    )
                ...
                if devices:
                    ...
                    if device_count != self._last_device_count:
                        _LOGGER.info(
                            f"Device count changed: {self._last_device_count} -> {device_count}"
                        )
                    ...
                    for device in devices:
                        ...
                        if device_id:
                            ...
                            if status:
                                ...
                                _LOGGER.debug(
                                    f"Updated status for {device_name} ({device_id})"
                                )
                                ...
                                if (
                                    ...
                                    if is_available:
                                        _LOGGER.info(
                                            f"Device {device_name} ({device_id}) is now ONLINE"
                                        )
                                    else:
                                        _LOGGER.warning(
                                            f"Device {device_name} ({device_id}) is now OFFLINE"
                                        )
                            ...
                            if schedules:
                                ...
                                schedule_count = (
                                    len(schedules.get("items", []))
                                    if isinstance(schedules, dict)
                                    else len(schedules)
                                )
                                _LOGGER.debug(
                                    f"Updated schedules for {device_name} ({device_id}): {schedule_count} schedules"
                                )
                    ...
                    _LOGGER.debug(
                        f"Successfully updated data for {len(device_statuses)} devices"
                    )
                else:
                ...
                if new_interval != self.update_interval.total_seconds():
                    self.update_interval = timedelta(seconds=new_interval)
                    _LOGGER.info(
                        f"Updated polling interval to {new_interval} seconds for public data only"
                    )
            ...
            # Add metadata
            data["auth_status"] = (
                "authenticated" if self.api.is_authenticated else "public_only"
            )
